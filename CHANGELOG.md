...
- Added `docs/custom-models.md` — llama.cpp model swap guide, API provider switching, and new LLMBackend implementation walkthrough
...
## [v0.4.0] — 2026-03-29 — Phase 4: UI + DuckDB audit
...
## [v0.3.0] — Phase 3: llama.cpp backend + GPU kernels

- `inference/llamacpp_backend.py`: async llama.cpp client over OpenAI-compatible endpoint (port 8080), exponential backoff retry
- `inference/router.py`: complexity-based routing — queries below 0.7 score use local 7B model, high-complexity queries escalate to cloud API
- `scripts/serve_llamacpp.sh` / `scripts/serve_llamacpp_turboquant.sh`: server launch scripts for RTX 4070 (IQ4_XS + q8_0 KV TurboQuant profile)
