...
Start a PostgreSQL + Redis for integration tests (optional — unit tests use in-memory SQLite):

```bash
# PostgreSQL
createdb mercer_dev
psql mercer_dev -f data/dvdrental/dvdrental.sql
...
- **Streaming inference** — token-level streaming from llama.cpp to the WebSocket endpoint
