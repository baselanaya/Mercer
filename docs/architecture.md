...
│                                              │
│  All three dispatched via asyncio.gather().  │
...
**Stage 4.** Three async tasks dispatched to `inference/llamacpp_backend.py` (or `inference/api_backend.py`) via `asyncio.gather()`. Each strategy uses its own prompt from `prompts/sql_generation.py`. Returns `List[SQLCandidate]` with metadata (strategy name, generation confidence, raw prompt).
...
```
complexity < 0.7  →  local  (Qwen2.5-Coder-7B-Instruct via llama.cpp)
complexity ≥ 0.7  →  api    (claude-sonnet-4-6 or gpt-4o)
```

### `inference/llamacpp_backend.py`

Async HTTP client to the llama.cpp server (OpenAI-compatible endpoint, default port 8080). Implements `generate(prompt)` and `generate_batch(prompts)`. Handles retries with exponential backoff via `tenacity`. Uses the TurboQuant profile by default: IQ4_XS quantized weights + q8_0 KV cache.

### `inference/api_backend.py`

Async clients for Anthropic and OpenAI APIs. Same interface as the llama.cpp backend — both implement the `LLMBackend` protocol defined in `inference/base.py`.
...
`config/inference.yaml` — llama.cpp server URL, model paths, quantization settings, complexity routing thresholds.

---

## llama.cpp Server

Mercer uses llama.cpp with GGUF quantization and a KV cache optimised for Text2SQL workloads. The TurboQuant profile fits a 7B model entirely within 8 GB VRAM:

| Component | Size |
|---|---|
| Weights (IQ4_XS) | ~4.5 GB |
...
```bash
# TurboQuant profile (IQ4_XS + q8_0 KV cache) — recommended
...
The server exposes an OpenAI-compatible endpoint on port 8080. Set `INFERENCE_BACKEND=llamacpp` in your environment (this is the default when `LLAMACPP_URL` is set).

Multi-candidate generation (Stage 4) sends all 3 prompts via `generate_batch()`. The asyncio coroutines are dispatched concurrently, so wall time ≈ single generation latency when the server is not saturated.

---

## Audit & Logging

Every pipeline run produces an `AuditEntry` written to the DuckDB audit log (`db/audit_store.py`). Fields: question, final SQL, tables used, columns used, model tier used, correction steps taken, total latency, per-stage timings.
...
```bash
# Print aggregate metrics (success rate, latency percentiles, correction rate)
